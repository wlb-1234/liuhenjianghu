...
# 微信支付
WX_MCHID=1114226626
WX_API_KEY=k9B2x7Q5rT1vF8nM3cD6hJ2kL9pQ4wE6
...
- **API 密钥 (API_KEY)**: `k9B2x7Q5rT1vF8nM3cD6hJ2kL9pQ4wE6` ✅ 已配置 (2026-07-30 21:45)
...
3. SSH 连接服务器
4. 执行 `git fetch origin` → `git reset --hard origin/main` → `pnpm install` → `node build.js` → `pm2 restart`
5. 部署完成

**优点**：全自动，无需人工干预，使用 `git fetch + git reset --hard` 避免分支分叉问题
...
> 最后更新：2026-08-02 22:45

### v1.0.23 (2026-08-02 22:45)

**全国行政区划数据导入完成 & 区域选择功能优化：**

1. **全国行政区划数据导入**
   - 导入民政部官方行政区划数据（44706 条记录）
   - 包含全国所有省级、地级、县级、乡级行政区
   - 支持港澳台地区（台湾省、香港特别行政区、澳门特别行政区）
   - 支持各种乡级行政区类型（街道、镇、乡、民族乡、苏木等）
   - 数据文件：`mca_official_regions.json`
   - 导入脚本：`server/scripts/import-regions.mjs`
...
3. **区域选择功能优化**
   - 首页默认显示会员最高等级对应的区域留言
   - 非会员（散人）默认显示注册镇级留言
   - 新增"省/市/县/镇"四级按钮，点击切换显示对应级别的留言
   - 新增"其他留言"按钮，可选择其他区域查看
   - 每级区域选择都有"确定"按钮，选择后显示对应区域的留言

4. **数据导入脚本修复**
   - 修复 `sort_order` 字段不存在的问题
   - 修复 `type` 字段 NOT NULL 约束问题
   - 根据 level 自动设置 type（省/市/县/镇）

5. **修改文件**
   - `client/screens/post/PostScreen.tsx` - 移除区域选择，使用用户注册区域
   - `client/screens/home/HomeScreen.tsx` - 优化区域选择功能
   - `client/services/api.ts` - 添加 regionCode 参数支持
   - `server/scripts/import-regions.mjs` - 修复导入脚本

6. **部署说明**
   - 需要在服务器上运行导入脚本：`npx tsx scripts/import-regions.mjs`
   - 导入完成后重启服务：`pm2 restart liuhen-api`

### v1.0.22 (2026-08-02 21:30)
...
## 2026-08-02 20:40 永久修复登录功能

### 问题原因
1. 昨晚修复的 SSL 配置和数据库连接问题今天又出现了
2. 根本原因：PM2 的 `pm2 save` 只保存进程列表，**不保存环境变量**
3. 服务器重启或 PM2 重启后，环境变量丢失
4. `.env` 文件可能被 Git 覆盖，回退到旧配置

### 永久修复方案
- 修改 `server/src/config/database.ts` 第 30 行
- 将 `ssl: { rejectUnauthorized: false }` 改为 `ssl: false`
- **硬编码 SSL 配置，不依赖环境变量**
- 这样即使 `.env` 被覆盖也不会影响 SSL 配置

### 修改文件
- `server/src/config/database.ts`（第 30 行）

### 测试结果
- ✅ 登录成功：`{"success":true,"message":"登录成功","token":"...","user":{...}}`
- ✅ 用户信息：id=1, phone=15613594588, nickname=壹号
- ✅ 返回 JWT token

### 教训
- PM2 的环境变量是临时的，`pm2 save` 不保存环境变量
- 关键配置应该硬编码到代码中，或确保 `.env` 文件不被 Git 跟踪
- 修改后必须提交到 GitHub，防止被覆盖
...
## 2026-08-04 23:30 会员体系与区域选择功能完善
...
|------|------|------|-----------|---------|---------|
| 0 | 江湖散人 | 免费 | 镇级 | 3 条/天 | 7 天 |
| 1 | 县级会员 | 20 元/月 | 县/镇 | 30 条/天 | 15 天 |
| 2 | 市级会员 | 100 元/月 | 市/县/镇 | 80 条/天 | 30 天 |
| 3 | 省级会员 | 500 元/月 | 省/市/县/镇 | 200 条/天 | 40 天 |
| 4 | 全国会员 | 2000 元/月 | 全国/省/市/县/镇 | 300 条/天 | 50 天 |
...
-- 插入新配置
INSERT INTO member_levels (level, name, price, daily_limit, retention_days, region_limit) VALUES
(0, '江湖散人', 0, 3, 7, 1),
(1, '县级会员', 20, 30, 15, 2),
(2, '市级会员', 100, 80, 30, 3),
(3, '省级会员', 500, 200, 40, 4),
(4, '全国会员', 2000, 300, 50, 5);
```

#### 前端代码修改
- `client/screens/profile/ProfileScreen.tsx`：更新会员等级显示逻辑
- `client/screens/post/PostScreen.tsx`：更新区域选择逻辑
...
#### 修改文件
- `client/screens/post/PostScreen.tsx`
  - availableRegions：根据会员等级生成可选区域列表
  - selectedRegionLevel：默认选中最高等级区域
  - 添加区域选择弹窗 UI
...
#### 问题 1：字段名不一致
- **问题**：前端使用 `user.membership_level`，数据库字段是 `member_level`
- **修复**：统一改为 `member_level`

#### 问题 2：等级判断逻辑错误
- **问题**：使用旧的 6 级体系判断（>=5, >=4, >=3, >=2）
- **修复**：改为新的 5 级体系（===4, ===3, ===2, ===1）

#### 问题 3：全国级别 region_level 错误
- **问题**：全国级别的 region_level 设置为 0
- **修复**：改为 5（与数据库一致）
...
#### 问题描述
- 新手机号注册时提示"该手机号已注册"
- 点击"立即登录"和"已有江湖身份"按钮无响应
- 新用户注册后发布区域显示"未设置区域"

#### 修复内容
1. **RegisterScreen.tsx**：添加注册失败时自动跳转到登录页面
2. **AuthContext.tsx**：修改 register 函数处理"已注册"错误
3. **userService.ts**：createUser 函数添加区域名称字段（province_name, city_name, district_name, town_name）
4. **auth.ts**：登录接口添加区域名称字段返回

#### 修改文件
- `client/screens/auth/RegisterScreen.tsx`
- `client/contexts/AuthContext.tsx`
...
#### 用户表字段
- `member_level`：会员等级（0-4）
- `province_code`：省级区划代码
- `province_name`：省级名称
- `city_code`：市级区划代码
- `city_name`：市级名称
- `district_code`：县级区划代码
- `district_name`：县级名称
- `town_code`：镇级区划代码
- `town_name`：镇级名称
...
#### 数据库更新
在阿里云 RDS DMS 控制台执行 SQL（见第一节）

#### 测试步骤
1. 强制刷新页面（Ctrl + Shift + R）
2. 用 15613594588 登录（全国会员）
3. 进入发布留言页面
4. 验证默认区域显示"全国"
5. 点击区域，验证可选择：全国/河北省/唐山市/迁安市/建昌营镇
6. 选择不同区域发布留言
7. 返回首页，切换到对应区域查看留言
...
1. **字段名一致性**：前后端字段名必须保持一致，避免使用不同的命名风格
2. **等级体系设计**：会员等级应该从 0 开始连续编号，避免跳号
3. **区域级别映射**：region_level 应该与会员等级对应，便于逻辑判断
4. **代码修改完整性**：修改等级体系时，需要检查所有相关的判断逻辑
5. **数据库配置**：会员等级配置应该统一管理，避免硬编码

---

## 2026-08-04 22:00 全国派会员等级添加

### 功能描述
添加最高等级"全国派"（后改为"全国会员"），支持在全国范围发布留言。

### 修改内容
1. 数据库添加 level=5 配置（后改为 level=4）
2. 前端添加全国派显示逻辑
3. 区域选择添加全国级别

### 后续调整
- 2026-08-04 23:30：将 6 级体系改为 5 级体系
- 等级名称从"天下共主"改为"全国会员"
- 价格从 0 元改为 2000 元/月

---

## 2026-08-04 21:00 发布留言区域选择功能初始实现

### 功能描述
移除发布留言页面的区域选择，改为默认使用用户注册区域。
...
### 2026-08-10 22:50 - 修复数据库连接问题

**问题描述**：
- 后端服务无法连接到阿里云 RDS 数据库（连接超时）
- 导致所有需要数据库的操作都失败（登录、发布帖子、获取帖子列表、点赞、评论等）
- 用户无法正常使用应用

**根本原因**：
- 阿里云 RDS 数据库（pgm-uf6sc0v55a1p3r7m.pg.rds.aliyuncs.com）无法从沙箱环境访问
- 可能是安全组/白名单配置限制

**解决方案**：
- 将 DATABASE_URL 从阿里云 RDS 切换到可访问的 volces.com 数据库
- 修改 `/workspace/projects/server/.env` 文件
- 重启后端服务

**修改文件**：
- `server/.env`: 更新 DATABASE_URL 配置
...
### 2026-08-18 21:31 - 建立 AI 辅助修改流程保护机制 ✅
...
## 2026-09-10 21:27 - 修复「关于我们」页会员体系展示为当前应用真实体系

### 背景
「我的」→「关于我们」→「会员服务」展示的「江湖会员体系」仍是**旧的武侠命名**（江湖散人 / 县帮帮主 9元 / 市盟盟主 19元 / 省派掌门 39元 / 天下会主 69元），与当前应用实际会员体系不一致，要求改为现在应用的会员体系。

### 根因
`screens/about/index.tsx` 中会员服务列表为**硬编码的旧版文案**（9/19/39/69 元 + 武侠命名），未随会员体系升级更新，也未与后端/「我的」页会员展示同步。

### 当前应用的会员体系（权威来源：`/api/v1/members/levels` 与 `ProfileScreen` 会员展示）
| 等级 | 发布范围 | 每日条数 | 内容留存 | 价格 |
|------|----------|----------|----------|------|
| 江湖散人 | 本镇 | 3 条/天 | 7 天 | 免费 |
| 县级会员 | 本县 | 30 条/天 | 15 天 | 20 元/月 |
| 市级会员 | 本市 | 80 条/天 | 30 天 | 100 元/月 |
| 省级会员 | 本省 | 200 条/天 | 40 天 | 500 元/月 |
| 全国会员 | 全国 | 300 条/天 | 50 天 | 2000 元/月 |

### 修复
将 `client/screens/about/index.tsx` 的「会员服务/会员体系」列表替换为上述当前应用的真实体系（散人 / 县级 / 市级 / 省级 / 全国，免费 / 20 / 100 / 500 / 2000 元）。

### 验证
- 文案与线上 `/api/v1/members/levels` 返回及「我的」页会员展示一致。
- 改动仅涉及 5 处字符串数组，构建无新增问题（`about` 页既有的 `Screen` props 类型提示为历史遗留，不影响 Expo 构建与运行）。
- 前端改动需在服务器 `git pull` + `npm run build` + `pm2 restart liuhen-client` 后对线上生效，用户线上确认修复成功。

### 交付
- `5240d1f` - fix(client): 关于我们-会员体系更新为当前应用的会员体系(散人/县市省全国会员)
...
## 2026-09-10 实名认证提交流畅化：放宽校验 + 表单内红字提示

### 问题现象
建表后提交认证不再报 500，但"填完信息点提交出现红色提示，控制台无日志、已填信息消失"。

### 定位
浏览器控制台在点击提交后**没有任何 POST 请求记录** → 请求被**前端本地校验拦截**（未发出），红色提示即拦截提示（原逻辑为身份证 18 位硬校验 `/^\d{17}[\dXx]$/`，且用原生弹出式提示）。

### 修复
1. **前端 `client/screens/realname/index.tsx`**
   - 校验放宽：姓名非空；身份证清洗（去空格、转大写）后允许 **15 位或 18 位** `/^(\d{15}|\d{17}[0-9X])$/`，减少误拦截。
   - 错误改为**表单内红字提示**（`errorText`），不再用原生弹窗打断，已填信息不被清空；仅提交成功才清空并跳转审核态。
2. **后端 `server/src/routes/realname.ts`**
   - 身份证校验同步放宽为 15/18 位，并统一清洗（去空格、转大写）后写入 `real_name` / `id_card`。

### 验证
- `node build.js` 后端构建通过；`screens/realname/index.tsx` 无新增类型错误（admin 页 TS7006 为历史遗留，非本次改动）。
- 提交 `78a6bd0` 已推送远端 main，后端经 GitHub Actions 自动重建重启。

### 交付
- `78a6bd0` - fix(realname): 放宽身份证校验(15/18位)并改用表单内红字提示, 避免误拦截且不弹窗清空

---

## 2026-09-12 实名认证产品化(第一步)：认证状态标识 + 引导入口

背景：明确实名认证的产品意义——作为内容合规底座 + 信任资产，采用"分层设计"：基础发帖开放，敏感/高价值功能后续逐步强制实名。

本轮交付(纯增量、低风险)：
1. **后端 `server/src/routes/auth.ts`**：`/auth/me`(及登录)返回新增 `verified` 字段——实时查询 `realname_verifications` 是否存在 `status=approved` 的记录。
2. **前端 `client/screens/profile/ProfileScreen.tsx`**：「我的」页个人信息区新增实名徽标：
   - 已实名 → 金色「已实名」徽标；
   - 未实名 → 「未实名 · 去认证」入口，点击跳转 `/realname` 引导认证（实现"认证引导"）。

验证：后端 `node build.js` 通过；前端仅剩 admin 页既有 `TS7006 reason` 历史错误（与本次无关），profile/realname 无新增错误。
