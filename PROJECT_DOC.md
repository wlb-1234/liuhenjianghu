...
> 最后更新：2026-07-07 21:30

### v1.0.12 (2026-07-07 21:30)

**数据库初始化接口：**

1. **新增数据库初始化接口**
   - `POST /api/v1/admin/init-db` - 创建缺失的数据表
   - 自动创建 `app_versions` 表
   - 用于生产环境首次部署时初始化数据库结构
...
1. **应用图标确定**
   ...
   - 风格：与登录页面黑黄主题呼应
   - 无文字，避免版权问题
   - 文件位置：`assets/app-store/icon/`

2. **图标文件清单**
   - `app-icon-1024.png` - 应用商店提交用（1024x1024）
   - `app-icon-final.svg` - 矢量源文件，可在 Figma/Illustrator 中编辑
   - `ICON_DESIGN_SPEC.md` - 设计规范文档（配色、尺寸、制作步骤）
...
1. **新增管理员接口**
   - `POST /api/v1/admin/init-db` - 初始化数据库表
   - 自动创建缺失的表（app_versions 等）
   - 生产环境部署后需调用此接口
...
1. **充值到账通知**
   - 支付成功后自动发送站内通知
   - 区分会员充值和余额充值，显示不同文案
   - 修改 `server/src/routes/payment.ts` 支付回调
...
2. **会员到期提醒**
   ...
   - 到期前1天：发送高优先级提醒
   - 到期当天：发送紧急提醒
   - 新建 `server/src/services/membershipExpiryService.ts`
   - 服务启动后5分钟执行首次检查，之后每24小时检查一次
...
**优化：**
- `notificationService.ts` 改用共享数据库连接池（移除硬编码凭据）
- 新增管理员接口 `POST /api/v1/admin/membership-expiry-check` 手动触发到期检查
...
**合规完善：**
- 用户协议添加「八、会员服务与退款政策」章节
- 会员升级页面添加退款提示
- 明确购买满24小时或使用核心功能后不支持退款

### v1.0.5 (2026-06-29 21:30)

**路由修复：**
- 移除 `post-detail` 显式路由声明，修复路由警告
- Expo Router 自动根据文件结构生成路由

### v1.0.4 (2026-06-29 21:15)

**认证修复：**
- 修复 token 存储 key 不一致问题
- `_layout.tsx` 使用正确的 key (`auth_token`) 检查登录状态
- 添加注册页路由豁免，避免跳转循环

### v1.0.3 (2026-06-29 21:00)

**跳转优化：**
- 优化未登录跳转逻辑
- 使用 `useSegments` 等待路由就绪
- 使用 `setTimeout` 确保在渲染周期后跳转

### v1.0.2 (2026-06-29 20:30)
